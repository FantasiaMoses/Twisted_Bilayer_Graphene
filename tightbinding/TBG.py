...
# dirac point
K_1 = np.array ([ 2 * np.pi / np.sqrt(3),  2 * np.pi / 3]) / L_M
K_2 = np.array ([ 2 * np.pi / np.sqrt(3), - 2 * np.pi / 3]) / L_M
...
# 4. 坐标轴与标题
plt.title(r"TBG Full Moire Bands at $\theta=1.05^\circ$ ($-300 \sim 300$ meV)", fontsize=18)
